...
export default class FEPaymentPlanMDP implements MDP {
  ...
  constructor({
    ...
    this.paymentCalculatorForm = new FPaymentCalculatorFFormMDP({
    ...
    return (nextCallback: () => void) => {
      if (
        ...
        if (editMode) {
          ...
          FSnackbar.error(
            "Total outstanding changed.Please recalculate to proceed"
          );
        } else {
          nextCallback();
        }
