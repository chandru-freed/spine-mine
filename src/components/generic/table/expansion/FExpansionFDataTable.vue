    ...
    >
      ...
      >
        <v-toolbar class="mx-1 py-0 ma-0" flat>
          ...
          <div v-if="groupByList.length>0">
            <v-menu z-index="99999" offset-y >
              ...
              <v-card max-height="400px">
                <v-list dense>
                  <v-list-item v-for="(item, index) in groupByList" :key="index" @click="handleGroupBySelect(item)">
                    <v-list-item-action >
                      {{item.text}}
                    </v-list-item-action>
                  </v-list-item>
        ...
        <v-toolbar flat class="align-right" v-if="showFilterForm">
          <v-chip small outlined label color="primary">Total Filtered: {{tableData().length}}</v-chip>
          <v-spacer />
          <div
      ...
      <template v-slot:[`group.header`]="{ group, headers, toggle, isOpen, items }">
        <td v-if="groupBy!==null" :colspan="headers.length">
          ...
          <span class="mx-5 font-weight-bold">{{groupBy.label}} ( {{items.length}} ) : {{ group }}</span>

          <span class="mx-3" v-if="groupBySummaryFunction"><strong>{{groupBySummaryFunction(items)}}</strong></span>
          <v-btn icon @click="resetGroupBy"><v-icon >mdi-close</v-icon></v-btn>
        </td>
      </template>
      <template
      ...
      >
        <slot :name="[`item.${header.value}`]" :item="item">
            ...
            ></component>
          <f-copy v-if="header.enableCopy" :value="selectModel(item,header.value)">
          </f-copy>
        </slot>
      </template>
...
export default class FExpansionFDataTable extends ModelVue {
  @Prop({
    default: () => [],
  })
  columnList: any[];
    ...
    // this.columnFilterListWithValues.forEach((columnFilter) => {
      filteredDataList = filteredDataList.filter((filteredData: any) => {
        if (columnFilter.booleanFilter === true) {
          console.log(columnFilter.value,"Filter value in boolean")
          return (
            columnFilter.value==undefined || columnFilter.value ===
            this.selectModel(filteredData, columnFilter.dataSelectorKey)
          );
        } else if (columnFilter.value.length > 0) {
          return columnFilter.value==undefined || columnFilter.value.includes(
            this.selectModel(filteredData, columnFilter.dataSelectorKey)
          );
        } else {
          return true;
