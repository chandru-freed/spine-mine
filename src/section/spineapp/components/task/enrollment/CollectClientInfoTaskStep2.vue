  ...
  <v-row>
    ...
    <v-col class="col-12" v-if="addDialog">
      <v-card outlined flat>
        <v-card-text class="pb-0">
          <component
            :ref="addFormMetaData.myRef"
            :is="addFormMetaData.componentName"
            v-model="addCreditorFormData"
            v-bind="addFormMetaData.props"
          ></component>
        </v-card-text>
      </v-card>
    </v-col>
    ...
    <v-col class="col-12" v-if="editDialog">
      <v-card outlined flat>
        <v-card-text class="pb-0">
          <component
            :ref="editFormMetaData.myRef"
            :is="editFormMetaData.componentName"
            v-model="editCreditorFormData"
            v-bind="editFormMetaData.props"
          ></component>
        </v-card-text>
      </v-card>
    </v-col>
...
export default class CollectClientInfoTaskStep2 extends ModelVue {
  ...
  addCreditor() {
    this.modelValue.push(this.addCreditorFormData);
  ...
  selectEditCreditor(item: any) {
    this.editCreditorFormData = {
      id: this.modelValue.indexOf(item),
      ...item,
    };
    this.showEditForm();
  }
  editCreditor(form: any) {
    Object.assign(
      this.modelValue[this.editCreditorFormData.id],
      this.editCreditorFormData
    );
    // this.onChange(); // calling saveTask
    this.closeAndClearAllForm();
  ...
  deleteCreditor() {
    this.modelValue.splice(this.deleteCreditorFormData.id, 1);
    // this.onChange(); // calling saveTask
    this.closeAndClearAllForm();
  }
  //DELETE ---- END
